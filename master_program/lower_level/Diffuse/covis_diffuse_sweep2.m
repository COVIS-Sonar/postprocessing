...
%function [covis] = covis_diffuse_sweep2(swp_path, swp_name, json_file, fig)
% Input
...
for np = 1:size(bf_sig_t,3)
    ...
    t1 = png(np).sec;
    t2 = t1+tlag;
    if ~isempty(find(abs(ping_sec-t2)<1/ping_rate,1))
        [~,np2] = min(abs(ping_sec-t2));
    else
        continue
    end
...
% calculate averages over a short window
